 ...
 **データの前提条件にミスがあったことを深くお詫び申し上げます。現在は、DRNの数理ロジックそのものの有効性を厳密に切り分けるため、よりシンプルな検証環境（Gridworld）へスケールダウンした検証と、アルゴリズムの改善策をリアルタイムで模索・実行中です。(2026年7月12日午前11時)**

 **追記：Gridworldでの検証結果(2026年7月12日午後17時)**

 <img src="./img/drn_midf.png" width="500px">

 >図1:中間報酬前

<img src="./img/drn_midt.png" width="500px">

>図2:中間報酬後

Gridworldでの検証の結果、DRNの数式自体はちゃんと収束することを確認いたしました。しっかりゴール(1,4)に向かいながら中間報酬(2,3)も取れています。図をよく見ると中間報酬(2,3)やゴール報酬(1,4)に近づくにつれて確率を分散させているのがわかります。これは報酬が目の前にあるとどっちみち取れるので、混合戦略を取っていると考えられます。つまり原理的にはナッシュ均衡を近似できるはずです。しかし、ハイパーパラメータの調整が難しく苦戦した為改善策を考える必要があり、次は重要度サンプリングを試しつつ7並べ環境を試したいと思います。


具体的なコードはこちらからご覧ください。src/agent/drn_agent.rsに更新方法やaction選択があります。
https://github.com/oyoyo4556/Sevens_Rust_RL

gridworldのコードやマップ・報酬の条件
https://github.com/oyoyo4556/rust_gridworld_DQN
