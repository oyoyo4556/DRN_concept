...
Gridworldでの検証の結果、DRNの数式自体はちゃんと収束することを確認いたしました。しっかりゴール(1,4)に向かいながら中間報酬(2,3)も取れています。図をよく見ると中間報酬(2,3)やゴール報酬(1,4)に近づくにつれて確率を分散させているのがわかります。これは報酬が目の前にあるとどっちみち取れるので、混合戦略を取っていると考えられます。つまり原理的にはナッシュ均衡を近似できるはずです。しかし、ハイパーパラメータの調整が難しく苦戦した為改善策を考える必要があります。次はのステップは2つ考えてあります。
1.重要度サンプリング
2.KLダイバージェンスをオフポリシーに持ち込む
