...
Gridworldでの検証の結果、DRNの数式自体はちゃんと収束することを確認いたしました。しっかりゴール(1,4)に向かいながら中間報酬(2,3)も取れています。図をよく見ると中間報酬(2,3)やゴール報酬(1,4)に近づくにつれて確率を分散させているのがわかります。これは報酬が目の前にあるとどっちみち取れるので、混合戦略を取れていると考えられる。つまり理論的にはナッシュ均衡を近似できると思われる。しかし、ハイパーパラメータの調整が難しく苦戦した為改善策を考える必要がある。とりあええず重要度サンプリングを試したいと思います。
